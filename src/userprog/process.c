...
static bool setup_stack (void **esp,char *fn,char **args,int argcount);
...
/* Loads an ELF executable from FILE_NAME into the current thread.
   ...
   SEA+ file_name can contain args and
   is parsed and args set up on stack.
 */
bool
  ...
  if (kpage != NULL) 
    {
      ...
      if (success) { //+SEA: No args, no wordbdry padding
	*esp = PHYS_BASE;
...
// round down to word boundary
        ...
        *(uint32_t *)*esp = (uint32_t)0;
        // copyt pointers to args onto stack (right to left!)
        for (i = argcount-1; i >= 0; i--) {
